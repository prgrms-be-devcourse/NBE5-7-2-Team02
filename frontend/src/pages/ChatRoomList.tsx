...
function ChatRoomList() {
    ...
    const fetchData = async () => {
        ...
        try {
            const res = await fetch("http://localhost:8080/api/chatroom", {
                // Adding cache control to ensure fresh data
                cache: "no-store",
                headers: {
            ...
            if (responseData && Array.isArray(responseData.data)) {
                // 서버 응답(ServerChatRoom[])을 클라이언트용 ChatRoom[]으로 매핑
                setItems(responseData.data)
            } else {
                setItems([])
                console.warn("API 응답에서 data 필드를 찾을 수 없거나 형식이 배열이 아닙니다.", responseData)
            }
        } catch (e: any) {
            console.error("채팅방 목록을 가져오는 중 오류 발생: ", e)
    ...
    return (
        // NavBar의 높이 고려하여 height: calc(100vh - 4rem) 사용
        <div className="flex h-[calc(100vh-4rem)] bg-[#f0f2f5] overflow-hidden p-4">
            {/* Chat Room List Panel - stays visible on the left */}
