...
public class MyFollowController {
    ...
    public ResponseEntity<BaseResponse<FollowCreateResponse>> follow(@AuthenticationPrincipal UserDetails userDetails,
        @PathVariable Long targetId) {
        FollowRequest followRequest = FollowRequest.of(Long.parseLong(userDetails.getUsername()), targetId);
        FollowCreateResponse response = followService.createFollow(followRequest);
        return BaseResponse.of(
                SuccessCode.CREATE_FOLLOWER,
                response,
                URI.create("/api/follow/me/followings"));
    }

    @DeleteMapping("/{targetId}")
    public ResponseEntity<BaseResponse<Void>> unfollow(@AuthenticationPrincipal UserDetails userDetails,
        @PathVariable Long targetId) {
        followService.deleteFollow(FollowRequest.of(Long.parseLong(userDetails.getUsername()), targetId));
        return BaseResponse.of(DELETE_FOLLOWING, null, null);
    }

    @GetMapping("/me/followers")
    public ResponseEntity<BaseResponse<Page<MemberCreateResponse>>> getFollowers(@AuthenticationPrincipal UserDetails userDetails,
        @PageableDefault(size = 20, sort = "id", direction = DESC) Pageable pageable) {
        Page<MemberCreateResponse> followers = followService.getFollowers(Long.parseLong(userDetails.getUsername()), pageable);
        return BaseResponse.of(FOUND_FOLLOWS, followers, null);
    }

    @GetMapping("/me/followings")
    public ResponseEntity<BaseResponse<Page<MemberCreateResponse>>> getFollowings(@AuthenticationPrincipal UserDetails userDetails,
        @PageableDefault(size = 20, sort = "id", direction = DESC) Pageable pageable) {
        Page<MemberCreateResponse> followings = followService.getFollowings(Long.parseLong(userDetails.getUsername()), pageable);
        return BaseResponse.of(FOUND_FOLLOWS, followings, null);
    }

    @GetMapping("/me/followers/count")
    public ResponseEntity<BaseResponse<Long>> getFollwersCount(@AuthenticationPrincipal UserDetails userDetails) {
    ...
    public ResponseEntity<BaseResponse<Long>> getFollwingsCount(@AuthenticationPrincipal UserDetails userDetails) {
