...
public class PostRepositoryFilterImpl implements PostRepositoryFilter {
    ...
    @Override
    public List<PostGetResult> findFilteredPosts(Long memberId, Long lastPostId, Integer limit, RecruitmentStatus recruitmentStatus,
        Boolean isFollowing, List<String> tags) {
    ...
    private BooleanExpression followingCondition(QPost post, Long memberId, Boolean isFollowing) {
        if (Boolean.TRUE.equals(isFollowing) && memberId != null) {
            return post.member.id.in(
                queryFactory.select(QFollow.follow.following.id)
                    .from(QFollow.follow)
        ...
        for (String tag : tags) {
            condition = condition.and(
                JPAExpressions.selectOne()
                    ...
                    .where(
                        QPostTag.postTag.post.eq(post),
                        QTag.tag.name.eq(tag)
                    ).exists()
            );
        }
        return condition;
